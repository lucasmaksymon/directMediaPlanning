...
export const EMAIL_FROM = process.env.EMAIL_FROM ?? "Direct Planning <noreply@directplanning.ar>";
...
export type EmailPayload =
  | { type: "welcome"; to: string; role: "advertiser" | "provider"; name?: string }
    ...
    switch (payload.type) {
      case "welcome":
        await resend.emails.send({
          from: EMAIL_FROM,
          to: payload.to,
          subject: "Bienvenido a Direct Planning",
...
<body>
  <div class="container">
    <div class="header">
      <div class="logo">Direct Planning</div>
    </div>
    <div class="body">
      ${body}
    </div>
    <div class="footer">
      Direct Planning · Marketplace OOH Argentina<br/>
...
function welcomeHtml(p: Extract<EmailPayload, { type: "welcome" }>) {
  const roleLabel = p.role === "provider" ? "medio/proveedor" : "anunciante";
  const dashboardUrl = `${APP_URL}/${p.role === "provider" ? "provider" : "advertiser"}`;
  return baseHtml("Bienvenido a Direct Planning", `
    <h2>Bienvenido a Direct Planning</h2>
    <p>Tu cuenta como <strong>${roleLabel}</strong> fue creada exitosamente${p.name ? ` para <strong>${p.name}</strong>` : ""}.</p>
    <p>Ya podés ${p.role === "provider" ? "cargar tus espacios publicitarios y recibir solicitudes de anunciantes." : "explorar el catálogo OOH y planificar tus campañas."}</p>
...
function newReservationHtml(p: Extract<EmailPayload, { type: "new_reservation" }>) {
  const url = `${APP_URL}/provider/reservations`;
  return baseHtml(`Nueva solicitud — ${p.unitName}`, `
    <h2>Nueva solicitud de reserva</h2>
    <p>Recibiste una solicitud de disponibilidad para uno de tus espacios.</p>
    <div class="data-box">
...
function reservationAcceptedHtml(p: Extract<EmailPayload, { type: "reservation_accepted" }>) {
  ...
  return baseHtml(`Solicitud aceptada — ${p.unitName}`, `
    ...
    <div class="data-box">
      <div class="data-row"><span class="data-label">Espacio</span><span class="data-value">${p.unitName}</span></div>
      <div class="data-row"><span class="data-label">Medio</span><span class="data-value">${p.providerName}</span></div>
      <div class="data-row"><span class="data-label">Desde</span><span class="data-value">${formatDate(p.startsAt)}</span></div>
      <div class="data-row"><span class="data-label">Hasta</span><span class="data-value">${formatDate(p.endsAt)}</span></div>
    </div>
